...
void loop()
{
	if(on_off == 0&&digitalRead(2)==1){//if the button is pressed and it previously wasn't pressed
		on_off = 1;//set it so it knows that the button was just on
		counter++;
		lcd.setCursor(0,1);
		lcd.print(counter);
	}
	if(on_off == 1&&digitalRead(2)==0){//if the button isn't on and it was just on
